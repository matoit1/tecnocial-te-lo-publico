...
Installing packages from apt.postgresql.org.
...
#------------------------------------------------------------------------------
# pg_hba.conf
echo "host all all 0.0.0.0/32 trust" >> /etc/postgresql/9.3/main/pg_hba.conf
echo "host all all 10.0.2.2/32 trust" >> /etc/postgresql/9.3/main/pg_hba.conf

# postgresql.conf
echo "listen_addresses = '*'" >> /etc/postgresql/9.3/main/postgresql.conf
PGCONFIG_DATA=$( cat <<EOF
...
echo "$PGCONFIG_DATA" >> /etc/postgresql/9.3/main/postgresql.conf

service postgresql restart

sudo -u postgres psql -c "CREATE ROLE tecnocial LOGIN PASSWORD 'tecnocial' SUPERUSER VALID UNTIL 'infinity';" &> /dev/null
sudo -u postgres psql -c "CREATE DATABASE tecnocial;" &> /dev/null